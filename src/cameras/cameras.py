...
class Camera:
    ...
    def get_image_plane_corners(self, focal_length: float | None = None) -> np.ndarray:
        """
        ...
        corners_cam = np.array(
            [
                [
                    (0 - cx) * focal_length / fx,
                    (0 - cy) * focal_length / fy,
                    focal_length,
                ],
                [
                    (width - cx) * focal_length / fx,
                    (0 - cy) * focal_length / fy,
                    focal_length,
                ],
                [
                    (width - cx) * focal_length / fx,
                    (height - cy) * focal_length / fy,
                    focal_length,
                ],
                [
                    (0 - cx) * focal_length / fx,
    ...
    def get_pyramid_with_texture_coords(
        self,
        focal_length: float | None = None,
        ...
        if self.image is not None and len(base_vertices) >= 4:
            height, width = self.image.shape[:2]
            height // pixel_sampling
            width // pixel_sampling
    ...
    def export_to_cloudcompare_ply(
        self,
        output_path: str,
        focal_length: float | None = None,
        pixel_sampling: int = 1,
        ...
        """
        try:
            vertices, _edges, _base_faces, _texture_coords, colors = (
                self.get_pyramid_with_texture_coords(
                    focal_length, pixel_sampling, image_sampling
                )
            )
            ...
            with open(output_path, "w") as f:
                ...
                for i, vertex in enumerate(vertices):
                    if i < len(colors):
                        ...
                        f.write(
                            f"{vertex[0]:.6f} {vertex[1]:.6f} {vertex[2]:.6f} {r} {g} {b} {alpha}\n"
                        )
                    else:
                        # Fallback color
                        f.write(
                            f"{vertex[0]:.6f} {vertex[1]:.6f} {vertex[2]:.6f} 255 255 255 255\n"
                        )
    ...
    def export_to_cloudcompare_obj(
...
def load_cameras_from_json(
    json_path: str, images_dir: str | None = None, scale_factor: float = 0.001
) -> list[Camera]:
    """
...
def convert_world_to_camera_to_camera_to_world(
    ...
    camera_center_in_world = -camera_to_world_rotation @ translation_vector.reshape(
        3,
    )
...
def create_cameras_from_stereo_calibration(
    calibration: dict,
    image_1: str | np.ndarray,
    image_2: str | np.ndarray,
    scale_factor: float = 0.001,
) -> tuple[Camera, Camera]:
    """
...
def export_cameras_to_cloudcompare(
    cameras: list[Camera],
    output_dir: str,
    format: str = "ply",
    pixel_sampling: int = 4,
    image_sampling: int = 4,
) -> bool:
    """
    ...
    for camera in cameras:
        if format == "ply":
            ...
            success = camera.export_to_cloudcompare_ply(
                output_path,
