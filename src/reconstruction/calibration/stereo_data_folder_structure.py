...
class StereoDataFolderStructure:
    ...
    def __init__(self, folder_path, admitted_folder_names: list[str] | None = None):
    ...
    def get_subfolders(self):
        ...
        return [
            f for f in os.listdir(self.folder_path) if f in self.admitted_folder_names
        ]
    ...
    def _check_scene_folder_structure(self, folder_path):
        ...
        for expected in expected_files:
            if expected not in actual_files:
                raise ValueError(
                    f"Expected '{expected}' in {folder_path}, but found: {actual_files}"
                )

        for expected in expected_folders:
            if expected not in actual_folders:
                raise ValueError(
                    f"Expected '{expected}' in {folder_path}, but found: {actual_folders}"
                )

        # check that the camera_1 and camera_2 folders have a video.mp4 file
        assert self._check_folder_has_one_video_file(
            os.path.join(folder_path, "camera_1")
        )
        assert self._check_folder_has_one_video_file(
            os.path.join(folder_path, "camera_2")
        )
        ...
        assert os.path.isfile(
            os.path.join(folder_path, "extrinsics_calibration_pattern_specs.yml")
        )
    ...
    def _check_calibration_intrinsics_folder_structure(self, folder_path):
        ...
        assert os.path.isfile(
            os.path.join(folder_path, "camera_1", "checkerboard_specs.yml")
        ), (
            f"checkerboard specs file not found in {os.path.join(folder_path, 'camera_1')}"
        )
        assert os.path.isfile(
            os.path.join(folder_path, "camera_2", "checkerboard_specs.yml")
        ), (
            f"checkerboard specs file not found in {os.path.join(folder_path, 'camera_2')}"
        )
    ...
    def _check_folder_has_one_video_file(self, folder_path):
        ...
        if len(video_files) != 1:
            raise ValueError(
                f"Folder {folder_path} has {len(video_files)} video files, expected 1"
            )
        return video_files[0]

    def check_folder_structure(self, name: str | None = None):
        self._check_folder_exists()
        self._check_subfolders_names()

        for subfolder in self.subfolders:
            if subfolder.startswith("scene"):
                self._check_scene_folder_structure(
                    os.path.join(self.folder_path, subfolder)
                )
            elif subfolder.startswith("calibration_intrinsics"):
                self._check_calibration_intrinsics_folder_structure(
                    os.path.join(self.folder_path, subfolder)
                )
            else:
                raise ValueError(f"Subfolder {subfolder} has an invalid name")
...
class SceneFolderStructure(StereoDataFolderStructure):
    ...
    def get_calibration_intrinsics_folder_name(self):
        with open(
            os.path.join(self.folder_path, self.scene_name, "parameters.yml")
        ) as f:
            parameters = yaml.safe_load(f)
        return parameters["calibration_intrinsics_folder_name"]


def load_stereo_data_folder_structure(
    stereo_data_folder_path: str | None = None,
) -> StereoDataFolderStructure:
    """
...
def load_scene_folder_structure(
    scene_name: str | None = None, stereo_data_folder_path: str | None = None
) -> SceneFolderStructure:
