...
def save_point_cloud_ply(points: np.ndarray, colors: np.ndarray, filename: str, bound_distance: float = 20) -> None:
    """
    ...
    logger.debug(f"Point cloud saved to {filename} with {len(points_valid)} points (filtered by distance > {bound_distance})")
