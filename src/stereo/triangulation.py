...
def triangulate_points(points1, points2, calib_data):
    ...
    K1, K2 = np.array(calib_data['camera_matrix1']), np.array(calib_data['camera_matrix2'])
    dist1, dist2 = np.array(calib_data['dist_coeffs1']), np.array(calib_data['dist_coeffs2'])
    R, t = np.array(calib_data['rotation_matrix']), np.array(calib_data['translation_vector'])
    ...
    logger.debug(f"3D points range: X[{points_3d[:, 0].min():.3f}, {points_3d[:, 0].max():.3f}], "
...
def extract_colors_from_image(points1, image):
    ...
    logger.debug(f"Extracted colors, range: [{colors_array.min()}, {colors_array.max()}]")
    return colors_array
...
def filter_pairs_with_mask(points1, points2, mask):
    ...
    logger.debug(f"Filtered pairs: {len(filtered_points1)} out of {len(points1)} points inside mask")
