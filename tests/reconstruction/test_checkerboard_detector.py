...
def test_checkerboard_detector_frame_0_should_fail():
    ...
    frame_path = (
        project_root / "assets" / "reconstruction" / "checkerboard" / "frame_0.png"
    )
    ...
    # Print results
    print(f"\n{'=' * 60}")
    print("Test: frame_0.png (should fail)")
...
def test_checkerboard_detector_frame_400_should_succeed():
    ...
    project_root = Path(__file__).resolve().parents[2]
    frame_path = (
        project_root / "assets" / "reconstruction" / "checkerboard" / "frame_400.png"
    )
    ...
    # Print results
    print(f"\n{'=' * 60}")
    print("Test: frame_400.png (should succeed)")
    print(f"Board size: {board_size}")
    print(f"Detection time: {detection_time * 1000:.2f} ms")
    print(f"Detection success: {success}")
    ...
    if success and corners is not None:
        ...
        cv2.drawChessboardCorners(processed_frame, board_size, corners, success)
