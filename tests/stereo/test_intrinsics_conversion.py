...
def generate_3d_test_points(camera_matrix, original_size, z_depth=1000.0, grid_spacing=200):
    """
...
def test_resize_image_dimensions(synthetic_checkerboard, target_size, patch_size, output_dir):
    ...
    assert height % patch_size == 0, f"Height {height} should be multiple of {patch_size}"
...
def test_intrinsics_conversion_accuracy(
    ...
    points_2d_ground_truth = apply_resize_crop_transform(points_2d_original, transform_params)
    ...
    print_sample_projections(points_2d_ground_truth_valid, points_2d_predicted_valid, pixel_errors)
    ...
    output_file = os.path.join(output_dir, f"test_accuracy_{intrinsics_fixture}_{point_method}.json")
    ...
    viz_file = os.path.join(output_dir, f"projections_{intrinsics_fixture}_{point_method}.png")
...
def test_corner_tracking_through_resize(
    ...
    all_corners = format_corners_for_json(corners_array, corners_transformed, valid_mask)
    ...
    resized_viz = create_corner_visualization(resized_image, corners_valid, (final_width, final_height))
...
def test_complete_pipeline_validation(
    ...
    for name, camera_matrix in [("intrinsics_1_1", intrinsics_1_1), ("intrinsics_1_2", intrinsics_1_2)]:
        print(f"\nTesting {name}...")
        ...
        print(f"  {status} - Max error: {max_error:.6f} px, Mean error: {mean_error:.6f} px")
