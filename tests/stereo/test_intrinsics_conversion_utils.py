...
def generate_3d_test_points_random(num_points=45, x_range=(-2000, 2000), y_range=(-1500, 1500), z_range=(500, 2000)):
    """
...
def create_projection_visualization(points_2d_ground_truth, points_2d_predicted, image_size):
    """
    ...
    for gt, pred in zip(points_2d_ground_truth, points_2d_predicted):
        ...
        draw.ellipse([gt[0]-3, gt[1]-3, gt[0]+3, gt[1]+3], fill=(0, 255, 0), outline=(0, 128, 0))
        # Predicted in red (smaller circle, should overlap)
        draw.ellipse([pred[0]-2, pred[1]-2, pred[0]+2, pred[1]+2], fill=(255, 0, 0), outline=(128, 0, 0))
...
def format_projections_for_json(points_2d_ground_truth, points_2d_predicted, pixel_errors):
    """
...
def print_sample_projections(points_2d_ground_truth, points_2d_predicted, pixel_errors, num_samples=5):
    ...
    for i in range(min(num_samples, len(points_2d_ground_truth))):
        ...
        pred = points_2d_predicted[i]
        error = pixel_errors[i]
        print(f"  Point {i}: GT=({gt[0]:.2f}, {gt[1]:.2f}), "
              f"Pred=({pred[0]:.2f}, {pred[1]:.2f}), Error={error:.4f}px")


def print_sample_corners(corners_original, corners_transformed, valid_mask, num_samples=5):
    ...
    for i in range(min(num_samples, len(corners_valid))):
        ...
        trans = corners_valid[i]
        print(f"  Corner {i}: Original=({orig[0]:.1f}, {orig[1]:.1f}) "
